...
with t1:
    ...
    with c_right:
        ...
        totals = sorted(
            [(c, round((df[c].dropna().iloc[-1] / df[c].dropna().iloc[0] - 1) * 100, 1))
             for c in companii if len(df[c].dropna()) >= 2],
            key=lambda x: x[1]
        )
    ...
    tabel = pd.DataFrame({
        "Preț start ($)":         [round(df[c].dropna().iloc[0], 2) if len(df[c].dropna()) >= 2 else None for c in companii],
        "Preț final ($)":          [round(df[c].dropna().iloc[-1], 2) if len(df[c].dropna()) >= 2 else None for c in companii],
        "Randament cumulat (%)":   [round((df[c].dropna().iloc[-1] / df[c].dropna().iloc[0] - 1) * 100, 1) if len(df[c].dropna()) >= 2 else None for c in companii],
